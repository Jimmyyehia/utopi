...
interface SchedulePageClientProps {
  initialData: {
    ...
    activeRoles: (UserTeamRole & { team: Team })[]
...
export function SchedulePageClient({ initialData }: SchedulePageClientProps) {
  ...
  const [rooms, setRooms] = useState<Room[]>(initialData.rooms.length > 0 ? initialData.rooms : DEFAULT_ROOMS)
  const [rawBookings, setRawBookings] = useState<BookingWithRelations[]>(initialData.bookings || [])
  const bookings = useRealtimeBookings(rawBookings)

  const [userRoles, setUserRoles] = useState<(UserTeamRole & { team: Team })[]>(initialData.activeRoles || [])
    ...
    async function refreshData() {
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data.bookings)) setRawBookings(data.bookings)
          if (Array.isArray(data.activeRoles)) setUserRoles(data.activeRoles)
        }
      } catch (err) {
  ...
  const handleBookingSubmit = async (data: any) => {
    ...
    if (rescheduleBookingId) {
      res = await fetch(`/api/bookings/${rescheduleBookingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...data,
    ...
    if (res.ok) {
      ...
      const isApproved = updatedBooking?.status === "APPROVED" || isManager
      setNotification({
        isOpen: true,
        title: isApproved ? "Reservation Confirmed! 🎉" : "Reschedule Request Submitted 🚀",
        message: isApproved
          ? "🎉 Your room reservation has been updated and confirmed!"
          : "🎉 Your reservation request has been updated and submitted for review. The prior version has been replaced.",
        type: "success",
      })
    } else {
      const err = await res.json().catch(() => ({}))
      setNotification({ isOpen: true, title: "Booking Failed", message: err.error || "Failed to process booking.", type: "error" })
    }
  }
  ...
  return (
    <div suppressHydrationWarning className="min-h-screen bg-background flex max-w-full overflow-x-hidden">
      {/* Universal Sidebar */}
      ...
      >
        ...
        {/* Page Content */}
        <div suppressHydrationWarning className="p-4 sm:p-6 max-w-7xl mx-auto w-full space-y-6">
          {/* Top Tabs & Filter Bar */}
                      ...
                      {dayBookings.map((b) => {
                        ...
                        return (
                          <motion.div
                            ...
                            data-context-menu-type="booking"
                            data-booking-id={b.id}
                            data-context-title={b.team?.name || "Reservation"}
                            data-context-subtitle={`${b.room?.name} • ${formatTime(start)} - ${formatTime(end)}`}
                            className={cn(
